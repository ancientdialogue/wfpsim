...
func (c *char) c1Init() {
	...
	c.Core.Events.Subscribe(event.OnEnemyHit, func(args ...any) bool {
		...
		if c.Core.Flags.LogDebug {
			c.Core.Log.NewEvent("Durin Quill proc dmg add", glog.LogPreDamageMod, atk.Info.ActorIndex).
				Write("before", atk.Info.FlatDmg).
				Write("addition", amt).
				Write("effect_ends_at", char.StatusExpiry(c1Key)).
				Write("quill_left", char.Tags[c1Key])
		}
...
func (c *char) c1OnBurst(isWhite bool) {
	...
	if isWhite {
		c.RemoveTag(c1Key) // 20 quill on hold
	...
	c.SetTag(c1Key, 20) // 20 quill on hold
	for _, char := range c.Core.Player.Chars() {
		if c.Index() == char.Index() {
			continue
		}
